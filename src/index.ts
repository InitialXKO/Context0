...
import { OffloadingManager } from "./offloadingManager.js"; // 导入 OffloadingManager
import { ContextProxy } from "./contextProxy.js"; // 导入 ContextProxy
...
const offloadingManager = new OffloadingManager(contextManager); // 初始化 OffloadingManager
const agentCore = new AgentCore(contextManager, offloadingManager); // 传入 offloadingManager
...
const contextProxy = new ContextProxy(server, agentCore); // 初始化 ContextProxy
...
server.registerTool(
  "saveContext",
  {
    ...
    inputSchema: z.object({ name: z.string(), content: z.string() }).shape,
  },
  async ({ name, content }) => {
...
server.registerTool(
  "executeTask",
  {
    ...
    inputSchema: z.object({ task: z.string() }).shape,
  },
  async ({ task }) => {
    // 模拟 AI 提供商的调用
    const aiProviderCall = async (injectedRequest: string): Promise<string> => {
...
server.registerTool(
  "offloadContent",
  {
    ...
    outputSchema: z.object({
      offloadedSegmentId: z.string().optional(),
      status: z.enum(["success", "failed", "partial"]),
      message: z.string().optional(),
      newContextReference: z.string().optional(),
    }).shape,
  },
...
server.registerTool(
  "retrieveWithDemand",
  {
    ...
    inputSchema: z.object({ id: z.string(), demandLevel: z.enum(["high", "medium", "low"]) }).shape,
  },
...
server.registerTool(
  "searchSegments",
  {
    ...
    inputSchema: z.object({ keywords: z.array(z.string()) }).shape,
  },
  async ({ keywords }) => {
