...
exports.OffloadingManager = void 0;
...
class OffloadingManager {
    ...
    async offloadContent(fullContent, parentId, summary, keywords) {
    ...
    getOffloadedSegmentByContent(content) {
    ...
    retrieveWithDemandLevel(id, demandLevel) {
        const segment = this.getSegment(id);
        if (!segment)
            return null;
        switch (demandLevel) {
    ...
    searchByKeywords(keywords) {
        const results = [];
        for (const segment of this.internalContext.values()) {
            const hasKeyword = keywords.some(keyword => segment.keywords?.some(kw => kw.toLowerCase().includes(keyword.toLowerCase())) ?? false);
            if (hasKeyword)
                results.push(segment);
        }
        return results;
    }
    retrievePartialDetails(id, query) {
        const segment = this.getSegment(id);
        if (!segment)
            return [];
        // Simple search in full_content
        const sentences = segment.full_content.split(/[.!?]/);
        return sentences.filter(sentence => sentence.toLowerCase().includes(query.toLowerCase())).map(s => s.trim());
    }
}
exports.OffloadingManager = OffloadingManager;
